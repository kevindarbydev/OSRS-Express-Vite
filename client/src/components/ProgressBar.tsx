...
const ProgressBar = ({ xp, level }: Props) => { 
  ...
  const progressBarValue =
    level === 99
      ? 100
      : 100 -
        (xpToNextLevel / (xpTNL[xpTNLIndex] - xpTNL[xpTNLIndex - 1])) * 100;
  return (
    <div>
      <span style={{ height: "42px", display: "block", textAlign: "center" }}>
        {level !== 99 ? (
          <>
            <Text
              variant="subtitle1"
              style={{ fontSize: "15px", marginTop: "1.5rem" }}
            >
              {xpToNextLevel} XP to {level + 1}
            </Text>
            <Progress variant="determinate" value={progressBarValue} />
            <Text
              variant="subtitle1"
              className="opacity-50 text-center text-sm"
            >
              {parseInt(progressBarValue.toFixed(0))}%
            </Text>
          </>
        ) : (
          <Text variant="subtitle1" style={{ marginTop: "1.76rem" }}>
            Level 99! 🎉{" "}
          </Text>
        )}
      </span>
